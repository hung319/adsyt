...
class ApiHelper:
    def __init__(self, config, web_session: ClientSession) -> None:
        ...
        # Cập nhật User-Agent
        self.web_session.headers.update({"User-Agent": self.modify_user_agent()})

    def modify_user_agent(self):
        """Chỉnh sửa User-Agent theo yêu cầu"""
        ua = self.web_session.headers.get("User-Agent", "")
        return ua.replace(")", "; Mediapartners-Google)") if ")" in ua else ua
    ...
    @list_to_tuple
    @AsyncConditionalTTL(time_to_live=300, maxsize=10)
    async def get_segments(self, vid_id):
        if await self.is_whitelisted(vid_id):
            return [], True
        vid_id_hashed = sha256(vid_id.encode("utf-8")).hexdigest()[:4]
        params = {
    ...
    def process_segments(response):
        ...
        try:
            ...
            for i in response_segments:
                ignore_ttl = ignore_ttl and i["locked"] == 1
                segment = i["segment"]
                UUID = i["UUID"]
                ...
                if segment_dict["start"] - segment_before_end < 1:
